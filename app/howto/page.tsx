...
const PANELS: { n: string; title: string; copy: React.ReactNode; art: React.ReactNode }[] = [
  {
    ...
    copy: (
      <>
        ...
        <Link href="/packs" className="underline decoration-pink underline-offset-2">
          packs page
        </Link>
        .
      </>
  ...
  },
  {
    ...
    copy: (
      <>
        ...
        <Link href="/binder" className="underline decoration-pink underline-offset-2">binder</Link>.
      </>
    ),
  },
  {
    ...
    copy: (
      <>
        Take any card to the <Link href="/arena" className="underline decoration-pink underline-offset-2">Arena</Link>.
  ...
  },
  {
    ...
    copy: (
      <>
        ...
        <Link href="/binder" className="underline decoration-pink underline-offset-2">binder</Link>, or
  ...
  },
  {
    ...
    copy: (
      <>
        <Link href="/create" className="underline decoration-pink underline-offset-2">
...
export default function HowToPage() {
  return (
    <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-10 sm:px-8">
      <header className="mb-8 border-b-[3px] border-line2 pb-4 text-center">
        <p className="micro text-[11px] tracking-[0.35em] text-pink">
          The full rules · there are not many
        </p>
        <h1 className="mt-1 text-3xl text-ink sm:text-4xl">How it works</h1>
      </header>

      <div className="grid gap-4 sm:grid-cols-2">
        {PANELS.map((p) => (
          <section key={p.n} className="surface-card flex items-start gap-4 p-5">
            <div className="shrink-0">{p.art}</div>
            <div className="min-w-0">
              <p className="micro text-[11px] font-semibold tracking-[0.3em] text-pink">
                {p.n} — {p.title}
              </p>
              <p className="mt-1.5 text-[15px] leading-snug text-ink2">{p.copy}</p>
            </div>
          </section>
        ))}
      </div>

      <Link href="/packs" className="rounded-[22px] border border-dashed border-line2 bg-surface mt-8 block p-5 text-center deal-in">
        <span className="bg-pink px-8 py-3 micro text-sm font-semibold text-on-accent hover:bg-pink">
          Rip your first pack →
        </span>
      </Link>
