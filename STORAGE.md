...
| `ai-index:peeked:v1` | `{ids: string[], total: number}` | `lib/binder.ts` | PEEKED stamps. `ids` cleared per-card on pull; `total` is lifetime. |
...
| `ai-index:storage-version` | number as string | `lib/storage.ts` | Migration cursor. v2 = stale keys cleared. |
...
## Removed keys (cleared by migration v2)
