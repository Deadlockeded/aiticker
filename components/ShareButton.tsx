...
export default function ShareButton({
  ...
  return (
    <button
      onClick={share}
      className={`rounded-full border border-white/20 px-6 py-2.5 font-semibold uppercase tracking-wide text-white/80 transition-colors hover:bg-white/10 ${className}`}
    >
      {copied ? "Copied ✓" : label}
    </button>
  );
