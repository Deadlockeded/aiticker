...
function Crest({ withRing }: { withRing: boolean }) {
  return (
    <svg viewBox="0 0 100 100" className="h-full w-full" aria-hidden>
      {withRing && (
        <>
          <circle cx="50" cy="50" r="46" fill="none" stroke="#F2EDE3" strokeWidth="1.6" />
          <circle cx="50" cy="50" r="30" fill="none" stroke="#F2EDE3" strokeWidth="1.2" />
          <defs>
      ...
      {!withRing && (
        <circle cx="50" cy="50" r="34" fill="none" stroke="#F2EDE3" strokeWidth="2.5" />
      )}
      <text
        x="50"
        y="50"
...
export default function CardBackFace({
  ...
  const frame = anonymous ? "#5A6070" : RARITY_FRAME[card.rarity];
  return (
    <div
      className={`card-back relative flex h-full w-full flex-col overflow-hidden rounded-[3px] bg-[#1E2430] ${thumb ? "border-2" : "border-[3px] paper-shadow"} ${artifact && !anonymous ? "border-dashed" : ""}`}
    ...
    >
      ...
      {!thumb && !anonymous && (
        <div className="relative border-t-2 bg-[#F2EDE3] px-2 py-1.5 text-center" style={{ borderColor: frame }}>
          <p className={`truncate font-mono uppercase tracking-widest text-[#1E2430] ${hero ? "text-sm" : "text-[10px]"}`}>
            {card.name}
          </p>
        </div>
      )}
