...
function Crest({ withRing }: { withRing: boolean }) {
  return (
    <svg viewBox="0 0 100 100" className="h-full w-full" aria-hidden>
      {withRing && (
        <>
          <circle cx="50" cy="50" r="46" fill="none" stroke="var(--surface)" strokeWidth="1.6" />
          <circle cx="50" cy="50" r="30" fill="none" stroke="var(--surface)" strokeWidth="1.2" />
          <defs>
      ...
      {!withRing && (
        <circle cx="50" cy="50" r="34" fill="none" stroke="var(--surface)" strokeWidth="2.5" />
      )}
      <text
        x="50"
        y="50"
...
export default function CardBackFace({
  ...
  const frame = anonymous ? "var(--ink2)" : RARITY_FRAME[card.rarity];
  return (
    <div
      className={`card-back relative flex h-full w-full flex-col overflow-hidden rounded-[3px] bg-ink ${thumb ? "border" : "border-[3px] shadow-card"} ${artifact && !anonymous ? "border-dashed" : ""}`}
    ...
    >
      ...
      {!anonymous && (
        <span
          className={`absolute right-1.5 top-1.5 rounded-sm px-1 font-mono uppercase tracking-wider text-on-accent ${RARITY_CHIP[card.rarity]} ${
      ...
      {!thumb && !anonymous && (
        <div className="relative border-t bg-surface px-2 py-1.5 text-center" style={{ borderColor: frame }}>
          <p className={`truncate font-mono uppercase tracking-widest text-ink ${hero ? "text-sm" : "text-[10px]"}`}>
            {card.name}
          </p>
        </div>
      )}
