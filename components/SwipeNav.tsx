...
export default function SwipeNav({
  ...
  return (
    <div className="pointer-events-none fixed inset-y-0 left-0 right-0 z-10 hidden items-center justify-between px-1 font-mono text-2xl text-white/15 [@media(hover:none)]:flex">
      <span>{prevId ? "‹" : ""}</span>
      <span>{nextId ? "›" : ""}</span>
    </div>
  );
